...
# ================== 配置 ==================
CSV_DIR = r"E:\2CODE\AaaaaIi\4FireFDS\2cavityPDE\T_400_0\steady\steady1_310K\TrainT40\newdataT40"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
FIXED_VIS_FILE = "Thot_40_0_pressure_m1_step34000.csv"
...
# 模型保存路径
MODEL_PATH = "best_pressure_delta_base24_bn.pth"
RESULT_DIR = "results_pressure_delta_base24_bn"
os.makedirs(RESULT_DIR, exist_ok=True)

I_MAX = 20
K_MAX = 20

# 修复 Padding，上下左右各补 2，变成 24x24 (被 8 整除)
PAD_DIMS = (2, 2, 2, 2)   # 20 -> 24
...
class PressureDeltaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = F.pad(inputs, PAD_DIMS, mode='replicate')
        
        # Target/Mask 等使用 0 填充，后续不会计入 Loss
        target_delta = F.pad(target_delta, PAD_DIMS, mode='constant', value=0.0)
        p_old = F.pad(p_old, PAD_DIMS, mode='constant', value=0.0)
        mask = F.pad(mask, PAD_DIMS, mode='constant', value=0.0)

        return inputs, target_delta, p_old, mask

    def _compute_stats(self):
        cache_path = os.path.join(os.path.dirname(self.files[0]), "delta_stats_cache_20x20.npz")
        if os.path.exists(cache_path):
            print("读取本地缓存的均值与方差数据（增量）...")
...
def plot_predictions_fixed_file(model, dataset, delta_mean, delta_std, epoch, device, target_filename):
    """
    固定使用某个 CSV 文件做可视化
    target_filename: 例如 'Thot_40_0_pressure_m1_step34000.csv'
    """
    model.eval()
    # 查找该文件名在 dataset.files 中的索引
    try:
        file_idx = [os.path.basename(f) for f in dataset.files].index(target_filename)
    except ValueError:
        print(f"警告：文件 {target_filename} 不在数据集中，跳过绘图。")
        return

    # 直接取该样本（已包含归一化 + padding）
    ...
    true_delta_np = target_delta[0].numpy()        # 注意去掉 batch 维
...
def main():
    ...
    # 划分训练集和验证集
    val_size = int(len(dataset) * VALID_SPLIT)  # 验证集大小
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # CosineAnnealingLR 直接 step() 即可，不依赖于 val_loss
        scheduler.step()
